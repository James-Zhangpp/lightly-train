"""将实例分割 checkpoint 导出为 ONNX（与 tools/train_inst.py 配套）。

ONNX 输入 images：float32 NCHW，ImageNet 归一化，尺寸为 export_meta.json 中的 image_size。
输出 labels, masks, scores：
  labels [N, Q]  每个 query 的类别 id
  masks  [N, Q, H, W]  二值 mask（模型输入分辨率）
  scores [N, Q]  置信度

同时写出 export_meta.json，供 infer_inst.py 使用。
"""
...
def export_onnx(
    ...
    *,
    ...
    print(
        ...
        f"(image_size={model.image_size}, num_queries={getattr(model, 'num_queries', '?')}, "
...
def smoke_test_onnx(onnx_path: Path, meta_path: Path, *, threshold: float = 0.5) -> None:
    ...
    x = (
        (dummy - torch.tensor(mean).view(3, 1, 1)) / torch.tensor(std).view(3, 1, 1)
    ).numpy().astype("float32")
    labels, masks, scores = session.run(None, {"images": x})
    keep = scores.reshape(-1) > threshold
    n = int(keep.sum())
    print(f"  instances (score>{threshold}): {n}")
...
def main() -> None:
    ...
    if not checkpoint.is_file():
        raise FileNotFoundError(
            f"Checkpoint not found: {checkpoint}\nTrain first: python tools/train_inst.py"
        )

    onnx_out = args.out.resolve() if args.out else checkpoint.parent / "model.onnx"
