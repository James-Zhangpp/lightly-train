"""将 A701_AVI 目标检测 checkpoint 导出为 ONNX（与 tools/train_A701_AVI_od.py 配套）。

调用 lightly_train 的导出方法，并生成 export_meta.json。
"""
...
def save_export_meta(model: torch.nn.Module, path: Path) -> None:
    classes = getattr(model, "classes", {0: "HD"})
...
def export_onnx(
    ...
    *,
    ...
    print(
        ...
        f"(image_size={getattr(model, 'image_size', (512,512))}, dynamic_batch={dynamic_batch})"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export A701_AVI object-detection checkpoint to ONNX",
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
...
def main() -> None:
    ...
    if not checkpoint.is_file():
        raise FileNotFoundError(
            ...
            "Train first: python tools/train_A701_AVI_od.py"
        )
